...
public class ClienteDAO {
    ...
    public void depositar(int clienteId, double valor, String tipoConta) {
        String sql = "UPDATE conta SET saldo = saldo + ? WHERE id_cliente = ? AND tipo_conta = ?";
    ...
    public boolean verificarClientePorUsuario(int idUsuario) throws SQLException {
        String query = "SELECT 1 FROM cliente WHERE id_usuario = ?";

        try (Connection connection = DBUtil.conectar();
             PreparedStatement stmt = connection.prepareStatement(query)) {
            stmt.setInt(1, idUsuario);
            ResultSet rs = stmt.executeQuery();
            return rs.next(); // Retorna true se o cliente for encontrado
        }
    }
}
